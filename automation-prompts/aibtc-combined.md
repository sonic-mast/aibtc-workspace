...
- **Bitcoin Optech newsletter (bitcoin-macro)** — PRIMARY: `https://bitcoinops.org/en/newsletters/YYYY/MM/DD/` for a specific dated newsletter. Anchor to a measurable claim *within* the issue (e.g. a fuse-filter speedup ratio), and follow Optech's link to the underlying primary (paper, PR, BIP) where available.- **tx-schemas / x402-sponsor-relay / agent-news / aibtc-mcp-server release (aibtc-network)** — PRIMARY: `https://github.com/aibtcdev/<repo>/releases/tag/<tag>` for release page, `https://github.com/aibtcdev/<repo>/pull/<num>` for a merged PR with measurable behavioral change. Cite commit SHA in body for traceability.
