...
- `estimate_inscription_fee` first. Run inscribe commit + reveal **in the same run, same turn** — never split across runs (#644: cross-process spawn truncates base64 content). Diff the commit response's `contentSize` against the local decoded byte length before reveal; mismatch = ABORT (a mismatched reveal permanently burns the reveal-address sats).
