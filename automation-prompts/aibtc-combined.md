...
Sonic Mast also *posts* bounties (Vibewatch Stacks M3 programme — `memory/vibewatch-m3-bounty-programme.md`). Those are ours to facilitate, never to hunt: the self-claim filter in B excludes them, and `bounty_submit` must never target a bounty whose `posterBtcAddress` is ours.
...
Read KV `m3Campaign` (`GET /kv/m3Campaign`). If `status` is `done`, or it's missing and today is after 2026-10-14, skip this phase silently. If it's missing before then, create it from the runbook's "Seed" section first. Otherwise read `automation-prompts/m3-campaign.md` and follow it exactly: the every-run checks (operator commands via comments on the `M3 campaign control` GitHub issue, from `brandonjamesmarshall` only; campaign replies; float detection), then the first due dated step. It is operator-approved (2026-09-15) for its listed autonomous actions only: templated paid inbox batches, free replies from its FAQ, Sonic Mast's capped window queries to Vibewatch's payTo, daily metrics, Tier 2 drafting, and Tier 2 posting after `approvals.postTier2`. Everything else in it is a ping to the operator, never an action: STX floats, acceptance, payouts. At most one paid-send batch per run; this phase runs even if Phase 4.5 used its action. PUT `m3Campaign` back once at the end and add an `m3` field to the run log.
