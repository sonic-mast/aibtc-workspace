...
- **Inbox send** (paid): `send_inbox_message_direct` (the sponsored `send_inbox_message` is deprecated; direct pays 100 sats + ~0.05 STX gas)
...
`bounties` is an array. **The build cap counts only `drafted` / `building` entries — max 3.** `submitted` entries do NOT consume a slot (operator fix 2026-08-07: they're in the poster's judging queue, cost nothing but a read-only monitor call, and were choking the pipeline by squatting build slots). Bound the total tracked array at 6 so judging pile-ups can't grow state without limit — if a 7th would enter, drop the stalest `submitted` entry to monitoring-via-`bountyHistory` only. Terminal outcomes (`won` / `abandoned`) are not stored here; they drop out of the array and are recorded in the `bountyHistory` KV ledger. Multi-day build bounties are explicitly allowed — they live across runs as `drafted`/`building` entries.
...
- **`drafted`**: build the deliverable (code repo or gist, writeup). Set its `status: "building"`, `lastActionAt: <iso>`. **Same-run fast path (small scope only):** if the deliverable is genuinely small — single file or gist, clear spec, finishable this run without rushing — continue straight through: build → local review gate (mandatory: fix `bug`s, one confirm re-run) → `bounty_submit`, all in this run. The review gate is what makes same-run shipping safe; a submission that skipped it doesn't qualify for the fast path. Fast path still counts as this run's ONE bounty action. Multi-day builds stay `building` across runs — log `bounty: "building <id>"` and stop here for this run.
...
**A0. Posted-bounty watch (poster side — read-only, every run, runs before A/B and never consumes the bounty action).**

Sonic Mast also *posts* bounties (Vibewatch Stacks M3 programme — `private/vibewatch-m3-bounty-programme.md`, local-only and gitignored). Those are ours to facilitate, never to hunt: the self-claim filter in B excludes them, and `bounty_submit` must never target a bounty whose `posterBtcAddress` is ours.

1. `bounty_my_posted` (read-only). For each non-terminal bounty, `bounty_get(id)` and diff `submissions[]` against the `postedBountyWatch` KV (`{"<bountyId>": {"seen": {"<submissionId>": {"precheck": "...", "at": "<iso>"}}}}`). Nothing new → skip the rest of A0 silently.
2. For each unseen submission, run the Hiro pre-check (read-only, no wallet): pull the txid(s) named in `message`/`contentUrl` from `https://api.hiro.so/extended/v1/tx/{txid}`. `pass` requires: anchored `success`; an sBTC (`SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token`) or STX transfer; recipient `SP3PHGPE8G09FFBSH6NVM3J5S2118M8YA825HWQY1`; amount ≥ 100 sats / 300,000 µSTX; sender = the submitter's STX address; sender is not ours (`SPG6VGJ5GTG5QKBV2ZV03219GSGH37PJGXQYXP47`, the payTo, or any Vibewatch-operated wallet) and has not already won the same tier. Tier 0 submissions (`bug`/`security` tags) get no pre-check beyond "contentUrl is a vibewatch-mcp issue" — record `precheck: "tier0-operator"`.
3. Record each new submission in `postedBountyWatch` with its result, log `notable: "posted-bounty submission <bountyId> <submissionId> precheck=<pass|fail:<reason>|tier0-operator>"`, and ping the operator: `scripts/telegram-send.sh "<bountyId> new submission from <submitterStx> — txid <txid> — precheck <result> — <contentUrl>"` (**always this script, never a bare `curl .../bot$TELEGRAM_BOT_TOKEN/...`** — the scheduled shell does not have the Telegram vars; the bare curl hits `/bot/sendMessage` and gets a 404 "Not Found". The script pulls them from `.env` and exits non-zero on any non-`ok` reply). If the send still fails, log `notable: "telegram send failed: <reply>"` — the `notable` line is enough (the daily digest surfaces it). If the submission describes a real defect (Tier 0, or a paragraph reporting a wrong 402/422, a payload served with no settled payment, or wrong docs), say so in the ping — the operator must confirm a fix before the next slot is posted.
4. **Never** `bounty_accept`, pay, or `bounty_cancel` from the loop. Acceptance and payout (memo `BNTY:{id}`) happen only in an interactive session after the operator's explicit "qualifies". The only posting the loop may do is the Tier 2 set, via Phase 4.6, and only once `m3Campaign.approvals.postTier2` is true.
5. All five T1 slots are paid/accepted — the series is closed. The full Tier 2 set is posted and live (`mu76082n33a9b60a9a9a`, `mu9zluf618779f56fc88`, `mu9zm5ci0e7e1cad96e5`; 10k sats each, expire 2026-10-09T17:00Z) — Phase 4.6 must NOT post further Tier 2 slots and `approvals.postTier2` stays false. Submissions from addresses in `postedBountyWatch.winners.tier1[]` on a T1 slot are `fail:already-won-tier1` — record them, do not ping.

**B. Top up the pipeline (only if `bounties` has < 3 entries in `drafted`/`building` — `submitted` entries don't count):**
...
4. **For the single best fit-score ≥3 candidate** (don't enqueue more than one new bounty per run): append `{ bountyId, status: "drafted", rewardSats, lastActionAt: <iso> }` to `bounties`. Log `bounty: "drafted <id> for <rewardSats> sats"`. **If Part A took no build action this run AND the new candidate qualifies for the small-scope fast path** (single file/gist, clear spec, finishable without rushing), you may run draft → build → review gate → submit in this same run — accepted-to-submitted in one turn, with the mandatory local review as the safety. Otherwise draft now, build next run. Never fast-path two bounties in one run.

Cap: **up to 3 in active build (`drafted`/`building`), unlimited-but-bounded-at-6 tracked overall, one state-advancing action per run.** `submitted` bounties ride free in judging and never block new work. This replaces the old one-at-a-time serialization — Sonic Mast should keep the pipeline full rather than deferring candidates to "later".
...
### Phase 4.6: Vibewatch M3 adoption campaign (conditional)

Read KV `m3Campaign` (`GET /kv/m3Campaign`). If `status` is `done`, or it's missing and today is after 2026-10-14, skip this phase silently. If it's missing before then, create it from the runbook's "Seed" section first. Otherwise read `private/m3-campaign.md` (local-only, gitignored — if it is missing, log `notable: "m3 runbook missing"` and skip) and follow it exactly: the every-run checks (operator decisions recorded in KV by interactive sessions; campaign replies; float detection), then the first due dated step. It is operator-approved (2026-09-15) for its listed autonomous actions only: templated paid inbox batches, free replies from its FAQ, Sonic Mast's capped window queries to Vibewatch's payTo, daily metrics, Tier 2 drafting, and Tier 2 posting after `approvals.postTier2`. Everything else in it is a ping to the operator, never an action: STX floats, acceptance, payouts. At most one paid-send batch per run; this phase runs even if Phase 4.5 used its action. PUT `m3Campaign` back once at the end and add an `m3` field to the run log.
...
- `GEMINI_API_KEY` must be exported (it's in `~/.zshenv`, so every local shell has it). Note the scheduled shell does **not** get everything in `.env` — `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in particular are absent, which is why Telegram pings go through `scripts/telegram-send.sh`.
