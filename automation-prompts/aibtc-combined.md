...
3. Record each new submission in `postedBountyWatch` with its result, log `notable: "posted-bounty submission <bountyId> <submissionId> precheck=<pass|fail:<reason>|tier0-operator>"`, and ping the operator: `curl -s "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" -d chat_id="$TELEGRAM_CHAT_ID" --data-urlencode text="<bountyId> new submission from <submitterStx> — txid <txid> — precheck <result> — <contentUrl>"`. If the Telegram send is blocked, the `notable` line is enough (the daily digest surfaces it). If the submission describes a real defect (Tier 0, or a paragraph reporting a wrong 402/422, a payload served with no settled payment, or wrong docs), say so in the ping — the operator must confirm a fix before the next slot is posted.
