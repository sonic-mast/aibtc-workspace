...
curl -s "https://bitcoinops.org/feed.xml" | python3 -c "import sys,re,json; t=sys.stdin.read(); items=re.findall(r'<entry[^>]*>.*?<title[^>]*>(.*?)</title>.*?<link[^>]*href=\"([^\"]+)\".*?<published>([^<]+)</published>.*?</entry>', t, re.S)[:4]; [print(json.dumps({'title':i[0],'url':i[1],'date':i[2]})) for i in items]" 2>/dev/null || echo "optech unavailable"
