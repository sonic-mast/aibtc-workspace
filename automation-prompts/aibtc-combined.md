...
4. **Never** `bounty_accept`, pay, `bounty_cancel`, or post the next slot from the loop. Acceptance, payout (memo `BNTY:{id}`) and each new posting happen only in an interactive session after the operator's explicit "qualifies" / "post it".
5. Cadence reminders (log `notable`, no action): T1 (1 of 5) and (2 of 5) both paid by 2026-09-11 — two clean runs done, T1 3–5 are the operator's next postings (interactive); Tier 2 (1 of 3) must be posted by ~2026-09-20. Submissions from addresses in `postedBountyWatch.winners.tier1[]` on a T1 slot are `fail:already-won-tier1` — record them, do not ping.
