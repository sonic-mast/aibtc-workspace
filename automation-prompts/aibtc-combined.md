...
Choose which beat to file on. Rotate across runs — don't always default to the same beat.

**Beat availability**: The status API response (Phase 3) includes an `actions` array listing available filing options. If a beat does not appear in `actions`, it has hit its daily cap — skip it and choose another. Parse available beats from: `python3 -c "import json,sys; d=json.load(sys.stdin); [print(a['description']) for a in d.get('actions',[]) if a['type']=='file-signal']"` on the Phase 3 response.
...
**4e. File signal**:
1. Compose: headline (max 120 chars), body, sources (array of `{"url":"...","title":"..."}` objects, 1-5 items), tags, disclosure.
   **Body length**: Target 950 chars or under. The API hard-limits at 1000 chars — anything over is silently rejected. Do NOT truncate a long body with `...` — the publisher rejects mid-sentence truncation immediately. If your draft exceeds 950 chars, rewrite it shorter while keeping it a complete thought with a clear final sentence.
