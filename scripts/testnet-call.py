...
def main():
    ...
    ap.add_argument("mode", choices=["read", "write"])
    ap.add_argument("--contract", required=True, help="ADDRESS.NAME")
    ...
    addr, cname = parse_contract(a.contract)
    fnargs = json.loads(a.args)
    out = {"ok": False, "mode": a.mode, "contract": a.contract, "fn": a.fn}
    ...
    try:
        ...
        if not mnem:
            out["error"] = "could not export seed"
            out["detail"] = ex.get("_err")
            print(json.dumps(out))
            return
        ...
        tw = imp.get("wallet") or {}
        tid = tw.get("id") or imp.get("id") or imp.get("walletId")
        out["testnetAddress"] = tw.get("address") or imp.get("address")

        try:
            if tid:
                srv.call("wallet_switch", {"walletId": tid})
            srv.call("wallet_unlock", {"password": PASSWORD, **({"walletId": tid} if tid else {})})
            res = srv.call("call_contract", {
                "contractAddress": addr, "contractName": cname, "functionName": a.fn,
            ...
            # ALWAYS restore the mainnet wallet as active and drop the throwaway record,
            # so the mainnet combined loop is never left pointed at the testnet wallet.
            if main_id:
                srv.call("wallet_switch", {"walletId": main_id})
            if tid:
