...
- [tmp scratch-file staleness](memory/tmp-scratch-file-staleness.md) — a classifier-blocked heredoc write left an older `/tmp` file in place; a later `curl -d @file` in the same run silently PATCHed 12h-stale values into state instead of erroring — re-verify artifacts after any transient tool-unavailable error, don't just retry the next step
