...
- `aibtc` — AIBTC wallet, signing, DeFi, identity tools (`@aibtc/mcp-server`, installed binary pinned in `.mcp.json`). **Prefer official MCP tools over custom curl** for any wallet / identity / bounty operation — see the "AIBTC MCP Operations" section in the combined prompt. aibtc.news: the `news_*` family is still 410 Gone (retired newsroom). `legion_*` reads (`legion_status`, `legion_list_stories`, `legion_get_story`, `legion_my_position`) are fixed as of at least 2026-08-22 and correctly resolve the live v7 mainnet contract (`SP5Y3W3F78NKFH4HYFNDQMJC484VZWKDH35ZR2M9.aibtc-news-gov`) — confirmed again 2026-08-25. `aibtc-mcp-server#649` is now CLOSED (fixed in #656 by pinning `src/config/legion.ts` to the single live mainnet era and deriving network from the contract address prefix, per aibtcdev/legions#12 2026-09-03) — prefer `legion_*` over `GET https://aibtc.news/api/state` for reads. `scripts/testnet-call.py` is legacy for the retired testnet v6 sandbox, not needed for v7 mainnet reads. Legion *writes* (`legion_contribute`/`legion_vote`/`legion_propose_story`/`legion_conclude`) go through the same MCP family — probe with a read first if in doubt.
...
- Code review is **local-first**: `scripts/gemini-review.py` (single-shot, REVIEW.md-calibrated) runs and findings are fixed BEFORE any `bounty_submit` or push; cubic (`cubic-dev-ai[bot]`) on PRs is the catch layer and review of record. `REVIEW.md` is the calibration SOT (risk tiers + feedback ratchet). Never invoke operator-billed reviewers (CodeRabbit `/code-review`, ultrareview) on Sonic Mast work — the operator's credits are reserved for his own projects.
