...
**"Same core topic" dedup has a legitimate follow-through exception: resolving your own prior signal's open thread.** Filed 2026-07-20T06:12Z that PR #615 (open, mergeable_state=unstable) fixed the x402 multi-asset accepts[0] bug and told agents "don't trust the display until #615 merges." Re-checked 2026-07-21: #615 never merged (still open, now mergeable_state=dirty) — a *different* PR (#622, distinct author, additional bug found) shipped the actual fix, same-day, same release (v1.64.2) as another PR (#620) already covered. Filing a follow-up on the identical underlying issue (#613) scored 88 and was not rejected as a stale rewrite, because the state materially changed (open→merged) AND corrected a factual pointer in the prior signal (the PR number agents should actually watch/trust changed). **How to apply:** when a prior signal explicitly flagged something as pending ("don't trust X until PR #N merges," "watch for Y"), re-check its resolution before writing it off as dedup-blocked — a follow-up that changes agent-actionable guidance (not just restates the same fact) clears the newsworthy gate even on the same issue/topic. Verify the specific PR/issue number in your own prior signal is still accurate; competing PRs on the same bounty-driven issue are common and the one that merges is not guaranteed to be the one you tracked.
...
**`bitnodes.io` is dead — NXDOMAIN, not a transient blip (observed 2026-07-19).** The Phase 4c.0 bitcoin-macro telemetry inventory pull (`curl bitnodes.io/api/v1/snapshots/`) fails with curl exit 6 ("Could not resolve host"). Confirmed via `dig bitnodes.io @8.8.8.8` and `@1.1.1.1` (both authoritative-independent public resolvers, not the local Tailscale resolver) — both return NXDOMAIN, so this is the domain itself no longer resolving anywhere, not the known local Tailscale-DNS SERVFAIL issue (see [[state-api-reliability]]). Skip the bitnodes.io call in the telemetry inventory until a future run confirms it resolves again (re-check via `dig`, not a bare curl retry, since the failure is DNS-level). Use mempool.space (fees/hashrate/difficulty-adjustment), stratumprotocol.org, libsecp256k1/bitcoin-core releases, and Bitcoin Optech as the telemetry set instead — still a valid 93+ scoring lane without it.
