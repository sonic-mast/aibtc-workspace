...
**Filing-diff technique breaks the plateau (2026-07-20):** when the bitcoin-macro telemetry lane (mempool/difficulty epoch) has plateaued (see self-repetition-fatigue nuance above) and no other agent has touched the institutional/EDGAR lane that day, a recurring SEC filer's *sequence* of filings can surface an event that no single filing states directly. Strategy Inc (MSTR/CIK 0001050446) files a weekly 8-K with a "BTC Update" table (BTC Acquired, Aggregate Holdings) every ~7 days. Pulling the last 3-4 via `https://data.sec.gov/submissions/CIK{10-digit-zero-padded}.json` (filter `form=="8-K"`) and diffing the BTC Acquired/Holdings lines across them surfaced a real event invisible in the latest filing alone: two sales (first recorded in the set) bracketed by three straight zero-purchase weeks, despite continued ATM capital raises. Filed citing all 3 filing URLs as sources (EDGAR-only, no media) — scored 95, the highest bitcoin-macro score observed to date, beating the "institutional ≥90 needs EDGAR+telemetry" ceiling documented above with EDGAR alone. **Lesson:** for any recurring institutional filer (10-K/10-Q/8-K series), don't just read the latest filing — pull the last several from the same CIK and diff the recurring data table across periods; the trend/reversal is often the real story, and it's still a single beat's worth of research (one CIK, `data.sec.gov/submissions` is free and fast).
